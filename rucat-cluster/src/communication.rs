...
impl Task {
  ...
  pub fn deserialize_many(buf: &[u8]) -> Result<Option<(NonEmptyVec<Task>, Vec<u8>)>> {
    match Self::deserialize(&buf)? {
      None => Ok(None),
      Some((hd, remainder)) => {
        match Self::deserialize_many(&remainder)? {
...
mod tests {
  ...
  fn deserialize_two() -> Result<()> {
    ...
    // parse one by one
    let (result1, remainder) = Task::deserialize(&buf)?.ok_or("should parse task1".to_owned())?;
    assert_eq!(result1, task1);
    let (result2, remainder) = Task::deserialize(&remainder)?.ok_or("should parse task1".to_owned())?;
    assert_eq!(result2, task2);
    assert!(remainder.is_empty());

    // parse many
    let (results, remainder) = Task::deserialize_many(&buf)?.ok_or("should parse 2 tasks".to_owned())?;
